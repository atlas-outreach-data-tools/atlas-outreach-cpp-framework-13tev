...
void SingleTopAnalysis::define_histograms()
{
  // HISTOGRAMS
  ...
  hist_leadjet_pt       = new TH1F("hist_leadjet_pt",      "Untagged Jet Transverse Momentum;p_{T}^{jet} [GeV];Events / bin", 20, 0, 200);
  hist_leadjet_eta      = new TH1F("hist_leadjet_eta",     "Untagged Jet Pseudorapidity; #eta^{jet}; Events / bin", 20, -3, 3);
  ...
  hist_leadleptE    = new TH1F("hist_leadleptE",   "Leading Lepton Energy; E^{leadlep} [GeV]; Events / bin", 15, 0, 300);
...
////////////////////////////////////////////////////////////////////////////////
void SingleTopAnalysis::FillOutputList()
{
  // histograms
  ...
  GetOutputList()->Add(hist_HT);
  GetOutputList()->Add(hist_Mlb);
  GetOutputList()->Add(hist_deltaeta);
...
////////////////////////////////////////////////////////////////////////////////
void SingleTopAnalysis::WriteHistograms()
{
  // histograms
...
void SingleTopAnalysis::FillHistogramsGlobal( double m, float w , TString s)
{
  //Fill Global histograms
  if (s.Contains("hist_etmiss")) hist_etmiss->Fill(m,w);
  if (s.Contains("hist_mtw")) hist_mtw->Fill(m,w);
  if (s.Contains("hist_deltaeta"))  hist_deltaeta->Fill(m,w);
  if (s.Contains("hist_HT")) hist_HT->Fill(m,w);
  if (s.Contains("hist_Mlb")) hist_Mlb->Fill(m,w);
}



void SingleTopAnalysis::FillHistogramsLeadlept( double m, float w , TString s)
{
  //Leading lepton histograms
...
void SingleTopAnalysis::FillHistogramsLeadJet( double m, float w , TString s)
{
