...
void TTbarAnalysis::define_histograms()
{
  ...
  hist_mtw          = new TH1F("hist_mtw",         "Transverse Mass; M^{W}_{T};Events", 40, 0, 200);

  // Jet variables histograms
  hist_n_jets           = new TH1F("hist_n_jets",          "Number of Jets;N_{jets};Events", 5, 3.5, 8.5);
  hist_leadjet_pt       = new TH1F("hist_leadjet_pt",      "Leading Jet Transverse Momentum;p_{T}^{jet} [GeV];Jets", 20, 0, 200);
  ...
  hist_leadbjet_pt      = new TH1F("hist_leadbjet_pt",     "Leading b-jet Transverse Momentum;p_{T}^{b-jet} [GeV];Jets", 20, 0, 200);
  hist_leadbjet_eta     = new TH1F("hist_leadbjet_eta",    "Leading b-jet Pseudorapidity; #eta^{b-jet}; Jets", 30, -3, 3);

  // W and top variables
  hist_Topmass       = new TH1F("hist_Topmass",      "Mass of three jets;m_{jjj}^{max p_{T}} [GeV];Jets", 40, 0, 300);
