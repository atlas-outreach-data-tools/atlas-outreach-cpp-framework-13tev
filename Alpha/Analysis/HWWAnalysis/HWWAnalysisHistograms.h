...
void HWWAnalysis::define_histograms()
{
  ...
  hist_mLL          = new TH1F("hist_mLL",       "Mass of Dilepton System; m_{ll} [GeV];Events", 30, 0, 170);
  hist_ptLL         = new TH1F("hist_ptLL",      "Transverse Momentum of Dilepton System; p_{T}^{ll} [GeV];Events", 30, 0, 200);
  hist_dPhi_LL      = new TH1F("hist_dPhi_LL",   "dPhi_LL Dilepton System; #Delta#phi (ll);Events", 20, 0, 3.2);
  hist_dPhiLLmet    = new TH1F("hist_dPhiLLmet", "dPhiLLmet Dilepton System; #Delta#phi (ll,E_{T,Miss});Events", 30, 0, 3.2);
  hist_etmiss       = new TH1F("hist_etmiss",    "Missing Transverse Momentum;E_{T,Miss} [GeV];Events", 20, 0,200);
  hist_mt           = new TH1F("hist_mt",        "Dilepton System Transverse Mass; m_{T};Events", 20, 50, 200);
  ...
  histF_mLL          = new TH1F("histF_mLL",       "Mass of Dilepton System; m_{ll} [GeV];Events", 30, 0, 170);
  histF_ptLL         = new TH1F("histF_ptLL",      "Transverse Momentum of Dilepton System; p_{T}^{ll} [GeV];Events", 30, 0, 200);
...
void HWWAnalysis::FillOutputList()
{
  ...
  GetOutputList()->Add(histF_etmiss);
  GetOutputList()->Add(histF_mLL);
  GetOutputList()->Add(histF_ptLL);
  GetOutputList()->Add(histF_dPhi_LL);
  GetOutputList()->Add(histF_dPhiLLmet);
  GetOutputList()->Add(histF_mt);
  ...
  GetOutputList()->Add(histF_n_jets);
  GetOutputList()->Add(histF_n_bjets);
...
void HWWAnalysis::FillHistogramsGlobal( double m, float w , TString s)
{
  ...
  if (s.Contains("histF_mLL"))    histF_mLL->Fill(m,w);
  if (s.Contains("histF_ptLL"))   histF_ptLL->Fill(m,w);
  if (s.Contains("histF_dPhi_LL")) histF_dPhi_LL->Fill(m,w);
  if (s.Contains("histF_etmiss")) histF_etmiss->Fill(m,w);
  if (s.Contains("histF_mt"))     histF_mt->Fill(m,w);
  if (s.Contains("histF_dPhiLLmet")) histF_dPhiLLmet->Fill(m,w);
  if (s.Contains("histF_n_jets")) histF_n_jets->Fill(m,w);
  if (s.Contains("histF_n_bjets")) histF_n_bjets->Fill(m,w);
