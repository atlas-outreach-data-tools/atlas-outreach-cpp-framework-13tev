...
void ZTauTauAnalysis::define_histograms()
{
  ...
  hist_mLL          = new TH1F("hist_mLL",         "Visible mass; m_{vis} [GeV];Events", 40, 1, 150);

  hist_MMC_etau     = new TH1F("hist_MMC_etau",    "Invariant mass (MMC); m^{MMC}_{e,#tau_{h}} [GeV];Events",   20, 68, 114);
  hist_MMC_mutau    = new TH1F("hist_MMC_mutau",   "Invariant mass (MMC); m^{MMC}_{#mu,#tau_{h}} [GeV];Events", 20, 68, 114);

  hist_mt_etau      = new TH1F("hist_mt_etau",      "Transverse Mass; m_{T}(e,E_{T,Miss}) [GeV];Events"  , 40, 1, 150);
  hist_mt_mutau     = new TH1F("hist_mt_mutau",     "Transverse Mass; m_{T}(#mu,E_{T,Miss}) [GeV];Events", 40, 1, 150);

  hist_sum_dPhi     = new TH1F("hist_sum_dPhi",     "sum dPhi; #Sigma#Delta#phi;Events", 40, 0, 6.3);
  ...
  hist_taupt  = new TH1F("hist_taupt", "Hadronic tau Transverse Momentum;p_{T}^{#tau_{h}} [GeV];Leptons", 20, 0, 200);
  hist_taueta = new TH1F("hist_taueta","Hadronic tau Pseudorapidity; #eta^{#tau_{h}}; Leptons", 10, -3, 3);
  hist_tauE   = new TH1F("hist_tauE",  "Hadronic tau Energy; E^{#tau_{h}} [GeV]; Leptons", 20, 0, 300);
  hist_tauphi = new TH1F("hist_tauphi","Hadronic tau Azimuthal Angle ; #phi^{#tau_{h}}; Leptons", 20, -3.2, 3.2);
  hist_tau_nTracks         = new TH1F("hist_tau_nTracks", "Hadronic tau number of tracks ; N_{tracks} (#tau_{h});Leptons", 5, -0.5, 4.5);
  hist_tau_BDTid         = new TH1F("hist_tau_BDTid", "Hadronic BDTid ; BDTid (#tau_{h});Leptons"      , 20, 0.6, 1);
