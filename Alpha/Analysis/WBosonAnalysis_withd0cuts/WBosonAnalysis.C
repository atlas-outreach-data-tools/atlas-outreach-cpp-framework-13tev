		  ...
		  if( lep_isTightID->at(i) )
		    {
		      ...
		      if( lep_pt->at(i) > 25000. && ( (lep_ptcone30->at(i)/lep_pt->at(i)) < 0.1) && ( (lep_etcone20->at(i) / lep_pt->at(i)) < 0.1 ) )
			{
			  ...
			  if ( lep_type->at(i)==11 && abs(lep_eta->at(i)<2.47) && ( abs(lep_eta->at(i) < 1.37) || abs(lep_eta->at(i) > 1.52) ) ) {
 		 	  ...
 		 	  if( abs(lep_trackd0pvunbiased->at(i))/lep_tracksigd0pvunbiased->at(i) > 5) continue;  
                          if( abs(lep_z0->at(i)*TMath::Sin(leptemp.Theta())) > 0.5) continue; 
		  ...
		  if(mtw > 50000.)
		    {
			  ...
			  double names_of_leadlep_variable[]={Lepton_1.Pt()/1000., Lepton_1.Eta(), Lepton_1.E()/1000., Lepton_1.Phi(), (double)lep_charge->at(goodlep_index), (double)lep_type->at(goodlep_index), abs(lep_trackd0pvunbiased->at(goodlep_index))/lep_tracksigd0pvunbiased->at(goodlep_index), abs(lep_z0->at(goodlep_index)*TMath::Sin(Lepton_1.Theta())) };
			  ...
			  TString histonames_of_leadlep_variable[]={"hist_leadleptpt", "hist_leadlepteta","hist_leadleptE","hist_leadleptphi","hist_leadleptch","hist_leadleptID","hist_leadlepd0","hist_leadlepz0"};
