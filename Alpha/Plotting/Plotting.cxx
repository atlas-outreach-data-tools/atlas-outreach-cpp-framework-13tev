...
#define YIELDS 0 // printing yields
...
int main(int argc, char *argv[]){
  ...
  if(argc < 3){
    std::cout<<"usage: ./plot [WBosonAnalysis, ZBosonAnalysis, TTbarAnalysis, WZDiBosonAnalysis, ZZDiBosonAnalysis, HWWAnalysis, HZZAnalysis, ZTauTauAnalysis, HyyAnalysis, SUSYAnalysis]  [location of OutputDir_AnalysisName]"<<std::endl;
  ...
  else { cout << "Analysis option not found! \n usage: ./plot [WBosonAnalysis, ZBosonAnalysis, TTbarAnalysis, HyyAnalysis, WZDiBosonAnalysis, ZZDiBosonAnalysis, HWWAnalysis, HZZAnalysis, SUSYAnalysis]  [location of OutputDir_AnalysisName] \n " << endl; exit(1);}
...
std::map<std::string,TH1F*>  lvvv;
  ...
  // reading Z Powheg samples 
  // as of now, they are used in "WBosonAnalysis" 
  if(option.find("WBosonAnalysis") != option.npos )
  {
    Zee = histo["Zee"];
    Zmumu = histo["Zmumu"];
  ...
  if(option.find("ZTauTauAnalysis") != option.npos ||   option.find("WBosonAnalysis") != option.npos ||   option.find("SUSYAnalysis") != option.npos)
  {
...
//      leg-> AddEntry(diboson , "Diboson", "f");
   ...
   if(option.find("SUSYAnalysis") != option.npos){
      ...
      leg-> AddEntry(stop, "Single top", "f");
      leg-> AddEntry(histstack,"Stat. unc.","f");
    //  leg-> AddEntry(slep600DM300_normsig, "m(l,#chi^{0}_{1})=(600,300)GeV" ,"l");
   }

    // only for SUSY
    TLegend* leg2;     leg2  = new TLegend();
    leg2  = new TLegend(0.45,0.45,0.93,0.50);
    leg2->SetFillStyle(0);
    ...
    leg2->SetTextAlign(32);
    if(option.find("SUSYAnalysis") != option.npos){
      leg2-> AddEntry(slep600DM300_normsig, "m(#tilde{l},#chi^{0}_{1})=(600,300) GeV" ,"l");
    }
    ...
    if(option.find("SUSYAnalysis") != option.npos){
       slep600DM300_normsig->Draw("HISTsame");
    }


    leg->Draw("same");
    if(option.find("SUSYAnalysis") != option.npos)     leg2->Draw("same");
