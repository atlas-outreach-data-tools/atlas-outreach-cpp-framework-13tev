...
void Plotting::WhichFiles()
{
    ...
    if (option.find("ZBosonAnalysis") != option.npos || option.find("TTbarAnalysis") != option.npos || option.find("WZDiBosonAnalysis") != option.npos || option.find("ZZDiBosonAnalysis") != option.npos)
    {
        ifile = "inputfiles/Files_base.txt";
    }
    ...
    {
        ifile = "inputfiles/Files_TTbarDilep.txt";
    }
    if (option.find("ZTauTauAnalysis") != option.npos)
    {
        ifile = "inputfiles/Files_ZTauTau.txt";
    }
    if (option.find("HyyAnalysis") != option.npos)
    {
        ifile = "inputfiles/Files_Hyy.txt";
    }
    if (option.find("HTauTauAnalysis") != option.npos)
    {
        ifile = "inputfiles/Files_HTauTau.txt";
    }
    ...
    while (getline(input, line))
    {
        if (line.find("#") != line.npos)
            continue; // a # is a comment and not read
        std::string name;
        ...
        if (DEBUG)
            cout << name << " " << xsec << " " << sumw << " " << eff << " " << kfac << endl;
...
void Plotting::readFiles()
{
    ...
    for (SFiter = SF.begin(); SFiter != SF.end(); SFiter++)
    {
        ...
        if (DEBUG)
            std::cout << "Reading file: " << readme << std::endl;
        TFile *file = TFile::Open(readme.c_str(), "READ");
        ...
        if (std::strstr((SFiter->first).c_str(), "data") == NULL)
        {
            ...
            for (unsigned int i = 0; i < v_n.size(); ++i)
            {
                ...
                {
                    if (DEBUG)
                        std::cout << "lumi " << lumi << "  xsec  " << SFiter->second.first << " sumw_eff " << SFiter->second.second << std::endl;
                    if (DEBUG)
                        std::cout << "Scaling histogram: " << fIter->first << " by a factor of: " << scf << std::endl;
...
void Plotting::makePlots()
{
    ...
    pad2X->SetLeftMargin(0.14);
    pad2X->SetRightMargin(0.05);
    ...
    for (; fIter != lIter; ++fIter)
    {
        ...
        if (option.find("HyyAnalysis") != option.npos)
        {
            if (fIter->first.find("_cat_") != option.npos)
                l.DrawLatex(0.18, 0.71, "H #rightarrow #gamma #gamma, unconv. central");
            else
                l.DrawLatex(0.18, 0.71, "H #rightarrow #gamma #gamma");
        }
        ...
        if (option.find("HyyAnalysis") != option.npos)
            leg = new TLegend(0.60, 0.45, 0.93, 0.825);
        // 0.7, 0.55, 0.85, 0.90
        leg->SetFillStyle(0);
        leg->SetBorderSize(0);
        leg->SetTextAlign(32);
        ...
        if (option.find("TTbarAnalysis") != option.npos || option.find("ZPrimeBoostedAnalysis") != option.npos)
        {
            ...
            if (YIELDS)
            {
                cout << "Yields:" << "Data: " << data[fIter->first]->Integral() << ",  single top: " << stop->Integral() << ",  ttbar: " << ttbar->Integral() << ",  V+jets: " << V->Integral() << ",  diboson: " << diboson->Integral() << ",  Total pred.: " << diboson->Integral() + V->Integral() + ttbar->Integral() + stop->Integral() << endl;
            }
        }
        ...
        if (option.find("HWWAnalysis") != option.npos)
        {
            ...
            leg->AddEntry(histstack, "Stat. unc.", "f");
            if (NORMSIG)
                if (fIter->first.find("histI") != option.npos)
                    leg->AddEntry(Higgs_normsig, "Higgs_{norm}", "l");

            if (YIELDS)
            {
                cout << "Number of events:" << "Data: " << data[fIter->first]->Integral() << ",  Higgs: " << Higgs->Integral() << ",  Diboson: " << diboson->Integral() << ",  V+jets: " << V->Integral() << ",  ttbar: " << ttbar->Integral() << ",  stop: " << stop->Integral() << ",  Total pred.: " << diboson->Integral() + V->Integral() + ttbar->Integral() + stop->Integral() << endl;
            }
        }
        ...
        if (option.find("ZTauTauAnalysis") != option.npos)
        {
            ...
            if (YIELDS)
            {
                cout << "Yields:" << "Data: " << data[fIter->first]->Integral() << ",  Z->tautau: " << Z_tautau->Integral() << ",  Z->ee,mumu: " << Z->Integral() << ",  W+jets: " << W->Integral() << ",  ttbar, single top, diboson: " << topV->Integral() << ",  Total pred.: " << Z->Integral() + Z_tautau->Integral() + W->Integral() + topV->Integral() << endl;
            }
        }
        ...
        if (option.find("TTbarDilepAnalysis") != option.npos)
        {
            ...
            if (YIELDS)
            {
                cout << "Yields:" << "Data: " << data[fIter->first]->Integral() << ",  ttbar: " << ttbar->Integral() << ",  single_top: " << stop->Integral() << ",  tt+others: " << tt_others->Integral() << ",  V+jets: " << V->Integral() << ",  diboson: " << diboson->Integral() << ",  Total pred.: " << tt_others->Integral() + diboson->Integral() + V->Integral() + ttbar->Integral() + stop->Integral() << endl;
            }
        }
        ...
        if (option.find("HTauTauAnalysis") != option.npos)
        {
            ...
            if (YIELDS)
            {
                cout << "Yields:" << "Data: " << data[fIter->first]->Integral() << ",  H->tautau: " << Hsig->Integral() << ",  Z->tautau: " << Z_tautau->Integral() << ",  Z->ee,mumu: " << Z->Integral() << ",  W+jets: " << W->Integral() << ",  ttbar, single top, diboson: " << topV->Integral() << ",  Total pred.: " << Hsig->Integral() + Z->Integral() + Z_tautau->Integral() + W->Integral() + topV->Integral() << endl;
            }
        }
        ...
        if (option.find("HyyAnalysis") != option.npos)
            h_ratio->GetYaxis()->SetNdivisions(503, false);
        ...
        if (option.find("HyyAnalysis") != option.npos)
        {
            if (fIter->first.find("_cat_") != option.npos)
            {
                h_ratio->SetMinimum(-30);
                h_ratio->SetMaximum(60);
            }
            else
            {
                h_ratio->SetMinimum(-300);
                h_ratio->SetMaximum(300);
            }
        }
        ...
        if (option.find("HyyAnalysis") != option.npos)
        {
            if (fIter->first.find("bin1") != option.npos)
            {
                if (fIter->first.find("_cat_") != option.npos)
                {
                    fIter->second->SetMaximum(3000);
                }
                else
                    fIter->second->SetMaximum(11e3);
            }
            if (fIter->first.find("bin2") != option.npos)
            {
                if (fIter->first.find("_cat_") != option.npos)
                {
                    fIter->second->SetMaximum(500);
                }
                else
                    fIter->second->SetMaximum(3000);
            }
...
void Plotting::getHistoSettings()
{
    ...
    if (option.find("TTbarAnalysis") != option.npos)
    {
        ifile = "list_histos/HistoList_TTbarAnalysis.txt";
    }
    if (option.find("HWWAnalysis") != option.npos)
    {
    ...
    if (option.find("ZTauTauAnalysis") != option.npos)
    {
        ifile = "list_histos/HistoList_ZTauTauAnalysis.txt";
    }
    if (option.find("TTbarDilepAnalysis") != option.npos)
    {
        ifile = "list_histos/HistoList_TTbarDilepAnalysis.txt";
    }
    if (option.find("HyyAnalysis") != option.npos)
    {
        ifile = "list_histos/HistoList_HyyAnalysis.txt";
    }
    if (option.find("HTauTauAnalysis") != option.npos)
    {
        ifile = "list_histos/HistoList_HTauTauAnalysis.txt";
    }
