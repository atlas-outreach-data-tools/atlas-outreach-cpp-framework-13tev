...
// standard lepton isolation requirement
  	if( lep_pt->at(i) >15000. && ( (lep_ptcone30->at(i)/lep_pt->at(i)) < 0.15) && ( (lep_etcone20->at(i) / lep_pt->at(i)) < 0.15 ) )
	{
		  ...
		  if ( lep_charge->at(goodlep1_index) * lep_charge->at(goodlep2_index)  < 0 ) 
		      ...
		      if ( lep_type->at(goodlep1_index) != lep_type->at(goodlep2_index) )
			{
			  ...
			  for(unsigned int i=0; i<jet_n; i++)
			    {
			      if(jet_pt->at(i)>20000. && abs(jet_eta->at(i)) < 4.5)
				{
				  // JVT cleaning
				  bool jvt_pass=true;
 ...
 TString histonames_of_global_variable_pre[]={"hist_mLL", "hist_ptLL", "hist_dPhi_LL", "hist_dPhiLLmet", "hist_etmiss", "hist_mt", "hist_n_jets", "hist_n_bjets"};
					 ...
					 if ( mLL < 55 )
					   {
						 ...
						 TString histonames_of_global_variable[]={"histF_mLL", "histF_ptLL", "histF_dPhi_LL", "histF_dPhiLLmet", "histF_etmiss", "histF_mt", "histF_n_jets", "histF_n_bjets"};
						 ...
						 for (int i=0; i<length_subleadlep; i++)
						   {
						     FillHistogramsSubleadlept( names_of_subleadlep_variable[i], weight, histonames_of_subleadlep_variable[i]);
			                         ...
			                         if ( goodjet_n == 0 ) FillHistogramsGlobal( mt, weight, "hist_mt");
